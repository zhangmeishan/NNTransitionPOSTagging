...
public:
	...
	void copy(const Feature& other) {
		...
		if (other._bStringFeat) {
			...
			for (int idx = 0; idx < other._strPostagFeat.size(); idx++) {
				_strPostagFeat.push_back(other._strPostagFeat[idx]);
			}
			...
			for (int idx = 0; idx < other._nPostagFeat.size(); idx++) {
				_nPostagFeat.push_back(other._nPostagFeat[idx]);
			}
