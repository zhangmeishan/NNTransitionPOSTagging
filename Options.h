...
public:
	...
	int postagEmbSize;
	int postagNgram;
	int postagHiddenSize;
	int postagRNNHiddenSize;
	...
	Options() {
		...
		postagEmbSize = 50;
		postagNgram = 2;
		postagHiddenSize = 100;
		postagRNNHiddenSize = 100;
	...
	void setOptions(const vector<string> &vecOption) {
		...
		for (; i < vecOption.size(); ++i) {
			...
			if (pr.first == "postagEmbSize")
				postagEmbSize = atoi(pr.second.c_str());
			if (pr.first == "postagNgram")
				postagNgram = atoi(pr.second.c_str());
			if (pr.first == "postagHiddenSize")
				postagHiddenSize = atoi(pr.second.c_str());
			if (pr.first == "postagRNNHiddenSize")
				postagRNNHiddenSize = atoi(pr.second.c_str());
			...
			if (pr.first == "times")
				times = atoi(pr.second.c_str());

			if (pr.first == "wordCutOff"){
				split_bychar(pr.second, vecInfo, ' ');
				for(int idx = 0; idx < vecInfo.size(); idx++){
					openTags.insert(vecInfo[idx]);
				}
	...
	void showOptions() {
		...
		std::cout << "postagEmbSize = " << postagEmbSize << std::endl;
		std::cout << "postagNgram = " << postagNgram << std::endl;
		std::cout << "postagHiddenSize = " << postagHiddenSize << std::endl;
		std::cout << "postagRNNHiddenSize = " << postagRNNHiddenSize << std::endl;
		...
		std::cout << "openTags =";
		for (hash_set<string>::iterator hashsetIt = openTags.begin(); hashsetIt != openTags.end(); hashsetIt++) {
			std::cout << " " << *hashsetIt;
		}
		std::cout << std::endl;
	}
